...
#=============================================================
# @(#)$Portal Version: pin_cmp_dm_oracle.pl:InstallVelocityInt:3:2005-May-12 18:13:17 $ 
# 
	...
	close( TMPFILE );
  	...
  	if ( $SETUP_CREATE_PARTITIONS =~ m/^YES$/i ) {
		...
		# Use ExecuteShell so that error will be handled by partition_utils and not directed to tmp.out
        	$Cmd = &FixSlashes( '/apps/brm/ThirdParty/perl/5.8.0/bin/perl '.$PIN_HOME.'/bin/partition_utils -o add -t realtime -s '.$startdate.' -u month -w 1 -q 12 -f' );
